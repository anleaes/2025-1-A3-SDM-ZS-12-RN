...
const EditVeiculoScreen = ({ route, navigation }: any) => {
  ...
  useEffect(() => {
    if (veiculo) {
      ...
      setCor(veiculo.cor);
      setValorDiaria(String(veiculo.valor_diaria));
      setCategoria(veiculo.categoria?.id ?? null);
      setAcessorios(veiculo.acessorios ? veiculo.acessorios.map(a => a.id) : []);
    }
    fetchCategorias();
    fetchAcessorios();
  }, [veiculo]);
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put(`/veiculo/${veiculo.id}/`, veiculoData);
      navigation.goBack();
    } catch (error) {
      if (typeof error === 'object' && error !== null) {
        const err = error as { response?: any; message?: string };
        console.log('Erro:', err.response?.data || err.message || err);
        Alert.alert('Erro', 'Não foi possível atualizar o veículo. ' + (err.response?.data?.detail || err.message));
      } else {
