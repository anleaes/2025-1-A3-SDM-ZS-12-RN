...
const CreateLocacaoScreen = ({ navigation }: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/locacao/', locacaoData);
      navigation.goBack();
    } catch (error) {
      if (typeof error === 'object' && error !== null) {
        const err = error as { response?: any; message?: string };
        console.log('Erro:', err.response?.data || err.message || err);
        Alert.alert('Erro', 'Não foi possível criar a locação. ' + (err.response?.data?.detail || err.message));
      } else {
...
export default CreateLocacaoScreen;