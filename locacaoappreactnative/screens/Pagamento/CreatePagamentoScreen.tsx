...
const CreatePagamentoScreen = ({ navigation }: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (typeof error === 'object' && error !== null) {
        ...
        Alert.alert('Erro', 'Não foi possível criar o pagamento. ' + (err.response?.data?.detail || err.message));
      } else {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        {!saving && <Button title="Salvar Pagamento" onPress={handleSave} color="#3498db" />}
