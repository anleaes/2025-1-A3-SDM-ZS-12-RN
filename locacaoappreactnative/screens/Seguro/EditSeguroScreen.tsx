...
const EditSeguroScreen = ({ route, navigation }: any) => {
  const { seguro } = route.params as { seguro: Seguro };
  ...
  useEffect(() => {
    if (seguro) {
      ...
      setValorDiario(String(seguro.valor_diario));
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put(`/seguro/${seguro.id}/`, seguroData);
      navigation.goBack();
    } catch (error) {
      if (typeof error === 'object' && error !== null) {
        const err = error as { response?: any; message?: string };
        console.log('Erro:', err.response?.data || err.message || err);
        Alert.alert('Erro', 'Não foi possível atualizar o seguro. ' + (err.response?.data?.detail || err.message));
      } else {
        console.log('Erro:', error);
        Alert.alert('Erro', 'Não foi possível atualizar o seguro.');
      }
    } finally {
      setSaving(false);
    }
