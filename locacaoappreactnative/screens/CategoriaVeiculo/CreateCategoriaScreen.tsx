...
const CreateCategoriaScreen = ({ navigation }: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/CategoriaVeiculo/', categoriaData);
      navigation.goBack();
    } catch (error) {
      Alert.alert('Erro', 'Não foi possível salvar a categoria. ' + error);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TextInput
        ...
        placeholder="Ex: Veículo utilitário esportivo"
        placeholderTextColor="#999"
      />
...
const styles = StyleSheet.create({
    ...
    input: { borderWidth: 1, borderColor: '#555', marginLeft: 280, borderRadius: 8, padding: 12, fontSize: 16, color: '#FFFFFF', marginBottom: 12 },
