...
const CategoriaVeiculoScreen = ({ navigation }: any) => {
  ...
  try {
    console.log('Enviando DELETE para:', `/CategoriaVeiculo/${id}/`);
    await api.delete(`/CategoriaVeiculo/${id}/`);
    setCategorias(prev => prev.filter(cat => cat.id !== id));
    console.log('Categoria removida com sucesso.');
  } catch (error: any) {
    console.log('Erro ao deletar categoria:', error.response?.data || error.message);
  }
};
