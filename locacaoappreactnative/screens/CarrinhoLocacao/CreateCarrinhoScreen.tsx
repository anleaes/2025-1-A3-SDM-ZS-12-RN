...
const CreateCarrinhoScreen = ({ navigation }: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/carrinholocacao/', carrinhoData);
      navigation.goBack();
    } catch (error) {
      if (typeof error === 'object' && error !== null) {
        const err = error as { response?: any; message?: string };
        console.log('Erro:', err.response?.data || err.message || err);
        Alert.alert('Erro', 'Não foi possível adicionar ao carrinho. ' + (err.response?.data?.detail || err.message));
      } else {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        {!saving && <Button title="Adicionar ao Carrinho" onPress={handleSave} color="#3498db" />}
...
export default CreateCarrinhoScreen;